...
function pickData(res: any, expectedStatus = 200) {
...
function bufferParser(res: any, callback: (err: any, body: Buffer) => void) {
  const chunks: Buffer[] = [];
...
async function getJson(method: string, url: string, token: string, query?: any) {
  let req = (api as any)[method](url).set(auth(token));
  if (query) req = req.query(query);
  const res = await req;
  return pickData(res);
}

async function sendJson(method: string, url: string, token: string | null, body: any, expectedStatus = 200) {
  let req = (api as any)[method](url);
  if (token) req = req.set(auth(token));
  const res = await req.send(body);
  return pickData(res, expectedStatus);
}

async function uploadFile(token: string, filename: string, buffer: Buffer, field = 'file') {
  const res = await api.post('/api/upload').set(auth(token)).attach(field, buffer, filename);
  return pickData(res);
}

async function importExcel(token: string, buffer: Buffer) {
  const res = await api.post('/api/import/members').set(auth(token)).attach('file', buffer, 'members.xlsx');
  return pickData(res);
}

async function exportBinary(url: string, token: string) {
...
async function main() {
  ...
  const memberRole = roles.find((item: any) => item.code === 'member');
  ...
  const roleData = await sendJson('post', '/api/system/roles', adminToken, {
    ...
    permissionIds: permissions.slice(0, 2).map((item: any) => item.id),
  });
  ...
  await sendJson('put', `/api/system/roles/${checkRoleId}`, adminToken, {
    ...
    permissionIds: permissions.slice(0, 3).map((item: any) => item.id),
  });
  ...
  assert.ok(usersPage.list.some((item: any) => item.id === sysUserId));
  ...
  assert.ok(linkableUsers.some((item: any) => item.id === memberUserId));
  ...
  assert.ok(Array.isArray(measurements) && measurements.some((item: any) => item.id === measurementId));
  ...
  assert.ok(Array.isArray(plans) && plans.some((item: any) => item.id === periodPlan.id));
  ...
  assert.ok(Array.isArray(checkIns.list) && checkIns.list.some((item: any) => item.id === manualCheckIn.id));
  ...
  assert.ok(coachList.list.some((item: any) => item.id === coachOpId));
  ...
  assert.ok(Array.isArray(courses) && courses.some((item: any) => item.id === groupCourse.id));
  ...
  assert.ok(myBookings.some((item: any) => item.id === selfBooking.id));
  ...
  assert.ok(Array.isArray(reviews.list) && reviews.list.some((item: any) => item.id === review.id));
  ...
  const unreadIds = notifications.list.filter((item: any) => item.is_read === 0).map((item: any) => item.id);
  ...
  assert.ok(Array.isArray(products.list) && products.list.some((item: any) => item.id === productOpId));
  ...
  assert.ok(storedHistory.list.some((item: any) => item.order_no === storedCheckout.order_no));
  ...
  assert.ok(Array.isArray(orders.list) && orders.list.some((item: any) => item.id === pendingOrderId));
  ...
  const settings: any = await getJson('get', '/api/system/settings', adminToken);
  const settingEntries: any[] = Object.values(settings).flatMap((items: any) => items);
