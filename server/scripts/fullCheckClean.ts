...
async function clean() {
  ...
  try {
    ...
    const memberUserIds = fcMembers.map((m) => m.user_id).filter(Boolean) as number[];

    const fcCoaches: any[] = await Coach.findAll({ where: { specialty: { [Op.like]: `%${COACH_SPECIALTY}%` } }, attributes: ['id', 'user_id'], transaction: t });
    const coachIds = fcCoaches.map((c: any) => c.id);
    const coachUserIds = fcCoaches.map((c: any) => c.user_id).filter(Boolean) as number[];
    ...
    const userRoleIds = fcUsers.map((u) => u.role_id).filter(Boolean) as number[];
