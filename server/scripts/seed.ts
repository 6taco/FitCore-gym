...
async function run() {
  ...
  for (const r of ROLES) {
    const [role] = await Role.findOrCreate({ where: { code: r.code }, defaults: r as any });
  ...
  for (const p of PERMISSIONS) {
    const [perm] = await Permission.findOrCreate({ where: { code: p.code }, defaults: p as any });
  ...
  for (const p of PLANS) {
    const [, c] = await MembershipPlan.findOrCreate({ where: { code: p.code }, defaults: p as any });
  ...
  if (coachCnt === 0) {
    ...
    for (const c of COACHES) await Coach.create({ ...c, status: 1 } as any);
  ...
  if (courseCnt === 0) {
    ...
    for (const c of COURSES) {
      const [row] = await Course.findOrCreate({ where: { code: c.code }, defaults: { ...c, status: 1 } as any });
  ...
  if (memberCnt === 0) {
    ...
    for (const d of demos) {
      await Member.create({ ...d, member_no: genNo('M'), status: 1 } as any);
    }
  ...
  if (memberUser) {
    ...
    if (!alreadyLinked) {
      let target: any = await Member.findOne({ where: { name: '张三' } });
      ...
      if (target) {
        ...
        const m = await Member.create({ name: '张三', gender: 1, phone: '13800000004', member_no: genNo('M'), status: 1, user_id: memberUser.id } as any);
  ...
  if (prodCnt === 0) {
    ...
    for (const p of PRODUCTS) await Product.create({ ...p, status: 1 } as any);
