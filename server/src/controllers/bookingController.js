...
/**
 * 预约课程
 * 并发安全：SELECT ... FOR UPDATE 行锁排期行；容量校验在锁内完成。
 */
...
/**
 * 取消预约（会员自己或前台）
 * 规则：仅 BOOKED 状态可取消；开课前 1 小时以内不允许（默认宽松策略：只限已开始的）
 */
...
/**
 * 签到（staff/coach 操作）
 * 若会员有活跃次卡（remaining_count > 0），自动扣 1 次；否则仍可签到（团课可能是期限卡/私教单独收费）
 */
...
export async function noShowBooking(req, res, next) {
  try {
...
/**
 * 会员自助预约（根据当前登录用户的 user_id 查找关联会员）
 */
export async function selfBook(req, res, next) {
  const t = await sequelize.transaction();
  try {
 ...
 * 会员自助取消预约
 */
export async function selfCancelBooking(req, res, next) {
  const t = await sequelize.transaction();
  try {
    const member = await Member.findOne({ where: { user_id: req.user.id }, transaction: t });
...
export async function myBookings(req, res, next) {
  try {
    ...
    const where = { member_id: Number(member_id) };
