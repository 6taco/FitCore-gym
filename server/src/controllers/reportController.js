...
// 日报 / 月报 统一接口
export async function dailySummary(req, res, next) {
  try {
  ...
  try {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const dayOrders = orders.filter(o =>
        o.status === 'PAID' && new Date(o.created_at) >= dayStart && new Date(o.created_at) <= dayEnd
      );
      dailyTrend.push({
...
// Dashboard 用：本月营收
export async function monthRevenue(_req, res, next) {
  try {
