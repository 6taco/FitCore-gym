...
export async function listSettings(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const grouped: Record<string, { key: string; value: string | null; label: string | null }[]> = {};
    for (const r of rows) {
...
// 获取单个设置值的工具函数
export async function getSettingValue(key: string, defaultValue: string | null = null): Promise<string | null> {
