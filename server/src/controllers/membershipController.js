...
function computeInitial(plan, startDate) {
  const start = dayjs(startDate || undefined).format('YYYY-MM-DD');
  const out = { start_date: start, end_date: null, remaining_count: null, balance: null };
  if (plan.type === 'PERIOD') {
    ...
    out.end_date = dayjs(start).add(365, 'day').format('YYYY-MM-DD'); // 次卡默认1年有效
...
export async function issueMembership(req, res, next) {
  try {
...
export async function resumeMembership(req, res, next) {
  try {
    const ms = await Membership.findByPk(req.params.id);
    if (!ms) throw new AppError('会员卡不存在', 404);
...
export async function transferMembership(req, res, next) {
  try {
...
// 即将过期 + 已过期但未标记
export async function expiringMemberships(req, res, next) {
  try {
...
// 批量续费
export async function batchRenew(req, res, next) {
  const t = await sequelize.transaction();
  try {
...
// 储值卡消费明细
export async function storedCardHistory(req, res, next) {
  try {
    ...
    res.json(success({
      ...
      list: rows.map((p) => ({
        ...
        status: p.status,
        paid_at: p.paid_at,
        items: (p.order?.items || []).map((i) => ({ name: i.item_name, qty: i.quantity, subtotal: i.subtotal })),
      })),
      total: count,
      page: Number(page),
