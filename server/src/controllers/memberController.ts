...
const coerceNum = (v: unknown) => (v === null || v === undefined || v === '' ? null : Number(v));
...
export async function listMembers(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const cntMap: Record<number, number> = activeCounts.reduce((acc: Record<number, number>, m) => {
...
// ============ 可关联账号列表 ============
export async function linkableUsers(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const where: any = { role_id: memberRole.id, status: 1 };
...
// ============ 会员卡 ============
export async function memberMemberships(req: Request, res: Response, next: NextFunction) {
  try {
...
// ============ 概要统计（Dashboard） ============
export async function memberStats(_req: Request, res: Response, next: NextFunction) {
  try {
