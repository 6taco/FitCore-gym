...
const qrTokenCache = new Map<string, QrSession>();
const bindTokenCache = new Map<string, BindSession>();
...
function formatQrSessionStatus(session: QrSession) {
  return {
...
function getQrSessionOrThrow(token: string): QrSession {
  cleanupCaches();
...
function markQrSessionSuccess(session: QrSession, memberName: string, checkInAt: Date = new Date()) {
...
function markQrSessionFailed(session: QrSession, message: string) {
  session.status = 'FAILED';
  session.errorMessage = message || '签到失败';
}

async function ensureMemberCanCheckIn(member: any, duplicateMsg: string) {
...
async function createCheckInRecord(member: any, method: string, remark: string | null, operatorId: number | null) {
...
export async function listCheckIns(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (start || end) {
      where.check_in_at = {} as any;
      if (start) where.check_in_at[Op.gte] = dayjs(start as string).startOf('day').toDate();
      if (end) where.check_in_at[Op.lte] = dayjs(end as string).endOf('day').toDate();
    }
...
export async function todayStats(req: Request, res: Response, next: NextFunction) {
  try {
    const todayStart = dayjs().startOf('day').toDate();
    const todayEnd = dayjs().endOf('day').toDate();
...
export async function generateQrToken(req: Request, res: Response, next: NextFunction) {
  try {
    cleanupCaches();
...
export async function getQrCheckInStatus(req: Request, res: Response, next: NextFunction) {
  try {
...
export async function qrCheckInByWechat(req: Request, res: Response, next: NextFunction) {
  let session: QrSession | undefined;
  try {
    const { token } = req.params;
    ...
    const member: any = await (Member as any).scope('withWechat').findOne({ where: { wechat_openid: openid } });
...
export async function bindWechatAndCheckIn(req: Request, res: Response, next: NextFunction) {
  let session: QrSession | undefined;
  try {
    ...
    const member: any = await (Member as any).scope('withWechat').findOne({
      where: { member_no: data.member_no.trim(), phone: data.phone.trim() },
    });
    if (!member) throw new AppError('会员编号或手机号不匹配', 400);
    if (member.status !== 1) throw new AppError('该会员已停用', 400);

    const occupied = await Member.findOne({ where: { wechat_openid: bind.openid } as any });
