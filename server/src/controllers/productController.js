...
export async function createProduct(req, res, next) {
  try {
    ...
    const p = await Product.create({ ...data, status: data.status ?? 1 });
...
// 库存变动（入库 / 出库 / 盘点调整）
export async function stockChange(req, res, next) {
...
export async function stockMovements(req, res, next) {
  try {
