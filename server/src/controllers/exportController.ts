...
export async function exportMembers(req: Request, res: Response, next: NextFunction) {
  try {
    const rows: any[] = await Member.findAll({
      include: [{
    ...
    for (const m of rows) {
      const activeCards = (m.memberships || [])
        ...
        .map((ms: any) => ms.plan?.name || ms.card_no)
        .join('、');
      ws.addRow({
...
export async function exportOrders(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const where: any = {};
    if (start) where.created_at = { ...(where.created_at || {}), [Op.gte]: dayjs(start as string).startOf('day').toDate() };
    if (end) where.created_at = { ...(where.created_at || {}), [Op.lte]: dayjs(end as string).endOf('day').toDate() };
    ...
    for (const o of rows) {
      ws.addRow({
        ...
        items: (o.items || []).map((i: any) => `${i.item_name}x${i.quantity}`).join('、'),
